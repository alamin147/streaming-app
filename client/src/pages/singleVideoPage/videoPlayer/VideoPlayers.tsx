...
export default function VideoPlayer({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full bg-black group ${isFullscreen ? "h-screen" : "aspect-video"}`}
      tabIndex={0}
    >
      {/* Video Element */}
      <video
      ...
      >
        ...
        <div className="px-4 pb-2">
          <Slider
            ...
            className="w-full cursor-pointer [&>span:first-child]:h-1 [&>span:first-child]:bg-white/30 [&_[role=slider]]:bg-red-500 [&_[role=slider]]:w-4 [&_[role=slider]]:h-4 [&_[role=slider]]:border-0 [&>span:first-child_span]:bg-red-500 [&_[role=slider]:focus-visible]:ring-0 [&_[role=slider]:focus-visible]:ring-offset-0 [&_[role=slider]:focus-visible]:scale-110 [&_[role=slider]:focus-visible]:transition-transform"
          />
        </div>
        ...
        <div className="flex items-center justify-between px-4 pb-4">
          <div className="flex items-center gap-2">
            ...
            {/* Skip Buttons */}
            <Button size="sm" variant="ghost" onClick={() => skip(-5)} className="text-white hover:bg-white/20 p-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="bg-black/90 border-white/20">
                <div className="px-2 py-1 text-white text-xs font-medium">Playback Speed</div>
                {[0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2].map((rate) => (
                  <DropdownMenuItem
                    key={rate}
                    onClick={() => changePlaybackRate(rate)}
                    className={`text-white hover:bg-white/20 ${playbackRate === rate ? "bg-white/10" : ""}`}
                  >
            ...
            {isPiPSupported && (
              <Button
                ...
                variant="ghost"
                onClick={togglePiP}
                className={`text-white hover:bg-white/20 p-2 ${isPiPActive ? "bg-white/10" : ""}`}
            ...
            {/* Fullscreen */}
            <Button size="sm" variant="ghost" onClick={toggleFullscreen} className="text-white hover:bg-white/20 p-2">
